...
use crate::manifest::{
    DEFAULT_XA_PERMISSIONS, DirectoryRecordPacking, Entry, EntrySectorSubheader, EntryXa,
    FileLayoutItem, GapKind, IdentifierPolicy, Iso9660, PathTableCopies, PrimaryVolume,
    PrimaryVolumeApplicationUse, RootDirectoryIdentifier, XaAttributes,
};
use crate::raw_cd::{LOGICAL_BLOCK_SIZE, XaSubheader};
...
pub fn parse(blocks: &[[u8; LOGICAL_BLOCK_SIZE]]) -> Result<ParsedIso> {
        ...
        .is_some_and(|block| &block[..7] == b"\x01CD001\x01")
    {
        ...
        matches!(primary_volume_copies, 1 | 3),
        "unsupported primary volume descriptor copy count {primary_volume_copies}"
    );
    let terminator_lba = 16 + primary_volume_copies;
    ensure!(
        blocks
            .get(terminator_lba)
    ...
    let (root_records, mut packing_observation) =
    ...
    while let Some((parent, extent, length)) = queue.pop_front() {
        ...
        let (records, observation) = read_directory(blocks, extent, length)?;
    ...
    let file_order = ordered
        ...
        .collect();
    let referenced_volume_space_size = directories
        .iter()
        .map(|directory| (directory.extent, directory.length))
        .chain(files.iter().map(|file| (file.extent, file.length)))
        .chain(
            entries
                .iter()
                .filter_map(|entry| entry.extent.zip(entry.length)),
        )
        .try_fold(0, |maximum, (extent, length)| -> Result<u32> {
            Ok(maximum.max(
                extent
                    .checked_add(length.div_ceil(LOGICAL_BLOCK_SIZE as u32))
                    .context("directory-record extent overflow")?,
            ))
        })?;
    ensure!(
        source_volume_space_size >= referenced_volume_space_size,
    ...
    Ok(ParsedIso {
        ...
        files,
        directories,
        path_tables: Some(ParsedPathTables {
...
fn read_directory(
    ...
    extent: u32,
    length: u32,
) -> Result<(Vec<Record>, DirectoryPackingObservation)> {
...
fn serialize_xa_system_use(entry: &Entry, directory: bool) -> Result<Vec<u8>> {
    ...
    let attributes = xa.and_then(|value| value.attributes).unwrap_or_else(|| {
        ...
        directory || !attributes.contains(crate::manifest::XaAttributeFlag::Directory),
        "file entry has XA directory attribute: {}",
        entry.path
    );
...
pub fn layout(iso: &Iso9660, file_lengths: &HashMap<String, u64>) -> Result<Layout> {
    ...
    let (path_table_pointers, mut next_extent) = match iso.path_table_copies {
        PathTableCopies::Duplicate => (
            [
                path_table_start,
                path_table_start + path_blocks,
                path_table_start + path_blocks * 2,
                path_table_start + path_blocks * 3,
            ],
            path_table_start + path_blocks * 4,
        ),
        PathTableCopies::Single => (
            [path_table_start, 0, path_table_start + path_blocks, 0],
            path_table_start + path_blocks * 2,
        ),
    };
    ...
    for (path, name, parent) in &directories {
        ...
        let blocks = packed_blocks(&record_lengths, iso.directory_record_packing) as u32;
    ...
    for item in &iso.files {
        if let Some(path) = item.as_directory() {
            ...
            for (sectors, kind, subheader) in pending_gaps.drain(..) {
        ...
        let Some(path) = item.as_path() else {
            ...
            match kind {
                GapKind::Form1 | GapKind::Form2 => {
                    pending_gaps.push((sectors, kind, item.gap_subheader()))
                }
    ...
    for (sectors, kind, subheader) in pending_gaps {
        gaps.push(GapPlacement {
            start: next_extent,
            sectors,
            kind,
    ...
    let external_files = iso
        ...
        .collect::<Vec<_>>();
    let referenced_content_blocks = placements
        .iter()
        .map(|placement| (placement.extent, placement.blocks))
        .chain(
            files
                .iter()
        ...
        .try_fold(0, |maximum, (extent, blocks)| -> Result<u32> {
            Ok(maximum.max(
                extent
                    .checked_add(blocks)
                    .context("authored or referenced extent overflow")?,
            ))
        })?;
    ...
    let logical_volume_blocks = if let Some(explicit) = iso.primary_volume.volume_space_size {
            ...
            explicit >= referenced_content_blocks,
    ...
    let terminator_lba = 16 + u32::from(iso.primary_volume_copies);
    blocks[usize::try_from(terminator_lba)?][0..7].copy_from_slice(b"\xffCD001\x01");
    write_path_tables(&mut blocks, &placements, path_table_pointers, path_blocks)?;
...
fn validate_entries(iso: &Iso9660) -> Result<HashSet<&str>> {
        ...
        matches!(iso.primary_volume_copies, 1 | 3),
        "primary_volume_copies must be 1 or 3"
    );
    let root = iso
        .entries
        .first()
    ...
    for (index, item) in iso.files.iter().enumerate() {
        ...
        }
        {
            ...
            let subheader = item.gap_subheader();
                ...
                previous_gap != Some((kind, subheader)),
            ...
            match kind {
                ...
                GapKind::Form2 | GapKind::Xa => ensure!(
                    subheader.is_none(),
    ...
    for (index, entry) in iso.entries.iter().enumerate() {
        ...
        let mixed_attributes =
            ...
            !mixed_attributes || (is_file && indexed_assets),
            "interleaved or Form 2 file attributes require indexed XA assets: {}",
            entry.path
        );
        ensure!(
...
fn make_record(
    ...
    make_record_with_padding(entry, extent, length, name, directory, false)
}
